...
result = df[(df['Остаток_в_магазине'] == 0) & (df['Остаток_на_складе'] > 2)]
result.insert(2, 'Рекомендовано к заказу', 1)
# Сохраняем результат
result[['Номенклатура', 'Рекомендовано к заказу', 'Остаток_в_магазине', 'Остаток_на_складе']].to_excel('заказ_пульты.xlsx', index=False)
redactor('заказ_пульты.xlsx')
