...
async def instruments():
    ...
    file = Path('C:/MyProjects/1c_scripts/остатки') / 'инструмент.xlsx'
    ...
    output_file = output_dir / f'Склад Инструмент от {current_date}.xlsx'
    if not result.empty:
    ...
    # Сохраняем файл
    filename = output_dir / f'Магазины Инструмент от {current_date}.xlsx'
    wb.save(filename)
