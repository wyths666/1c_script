...
async def batteries():
    ...
    file = Path('C:/MyProjects/1c_scripts/остатки') / 'батарейки.xlsx'
    ...
    output_file = output_dir / f'Склад Батарейки от {current_date}.xlsx'
    if not result.empty:
    ...
    # Сохраняем файл
    filename = output_dir / f'Магазины Батарейки от {current_date}.xlsx'
    wb.save(filename)
