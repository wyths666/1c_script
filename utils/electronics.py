...
async def electronics():
    global df
    start_time = time.perf_counter()
    current_date = datetime.now().strftime('%d-%m-%Y')
    output_dir = Path('C:/MyProjects/1c_scripts/отчеты')
    output_dir.mkdir(exist_ok=True)
    file = Path('C:/MyProjects/1c_scripts/остатки') / 'электроника.xlsx'
    ...
    output_file = output_dir / f'Склад Электроника от {current_date}.xlsx'
    if not result.empty:
    ...
    # Сохраняем файл
    filename = output_dir / f'Магазины Электроника от {current_date}.xlsx'
    wb.save(filename)
